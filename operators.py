...
def move_camera_to_target(dx, dy):
    ...
    if dx < -5: # - право
        command = bytearray([0xFF, 0x01, 0x00, 0x02, 0x05, 0x00, 0x08])

    if dx > 5: # + лево
        command = bytearray([0xFF, 0x01, 0x00, 0x04, 0x05, 0x00, 0x0A])

    if dy < -5: # - вниз
        command = bytearray([0xFF, 0x01, 0x00, 0x10, 0x05, 0x00, 0x16])

    if dy > 5: # + вверх
        command = bytearray([0xFF, 0x01, 0x00, 0x08, 0x05, 0x00, 0x0E])
