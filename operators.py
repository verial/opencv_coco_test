...
def move_camera_to_target(dx, dy):
    ...
    if dx < -5: # - право
        command = bytearray([0xFF, 0x01, 0x00, 0x02, 0x64, 0x00, 0x67])

    if dx > 5: # + лево
        command = bytearray([0xFF, 0x01, 0x00, 0x04, 0x64, 0x00, 0x69])

    if dy < -5: # + вниз
        command = bytearray([0xFF, 0x01, 0x00, 0x10, 0x64, 0x00, 0x75])

    if dy > 5: # - вверх
        command = bytearray([0xFF, 0x01, 0x00, 0x08, 0x64, 0x00, 0x6D])
